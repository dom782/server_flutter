        ...
        if (msg.type === 'auth_response') {
          ...
          if (pending) {
            ...
            if (msg.ok) {
              ...
              pending.ws.token    = msg.token; // salva JWT per verifica
              console.log(`[AUTH] ✓ ${msg.username} (${msg.ruolo}) token=${msg.token ? 'ok' : 'no'}`);
      ...
      if (type === 'logout') {
        if (serviceClient && serviceClient.readyState === 1) {
          serviceClient.send(JSON.stringify({
            type:  'logout',
            token: msg.token || '',
            from:  ws.username || clientName,
          }));
        }
        ws.send(JSON.stringify({ type: 'logout_response', ok: true }));
        return;
      }
...
server.listen(PORT, () => console.log(`ScanPC server v11 avviato sulla porta ${PORT}`));
